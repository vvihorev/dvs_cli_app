...
class Criterion(object):
    def __init__(self, engine_parameters: pd.DataFrame, base_vibration_level: float):
        ...
        self.results: CalculationResults = self._create_multiindexed_frame()
    ...
    def _create_multiindexed_frame(self) -> CalculationResults:
        arrays = [
        ...
        df_vibrations = pd.DataFrame(index=self.engine_parameters.name, columns=FREQUENCIES)
...
class SecondCriterion(Criterion):
    ...
    def process_engine(self, engine: Engine) -> None:
        ...
        for group in self.engine_parameters.group.unique():
            df_group = self.engine_parameters[self.engine_parameters.group == group].copy()
